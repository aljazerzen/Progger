...
public:
	...
	bool getPackageDone(unsigned int pack);
...
void Menu::makeTexture(int eleIndex) {
	...
	case 3:
		if ( packStructure.getPackageDone( ele[eleIndex].value & 0xFF))
...
bool Structure::getPackageDone(unsigned int pack) {
	if (pack >= packages.size() || pack < 0) pack = 0;
	return packages[pack]->getDone();
}
...
void PopupHandler::addPopup(char *text) {
	...
	while (!done) {
		...
		for (unsigned int i = 0; i < popups.size(); i++) {
			if ( popups[i]->isActive() && popups[i]->getBox().y == y ) {
...
void genHelp(int hn) {
	...
	switch (hn) {
	case 1:
		canvas = SDL_CreateRGBSurface(0, 150 * msf, 115 * msf, SCREEN_BPP, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
		SDL_FillRect(canvas, NULL, SDL_MapRGBA(canvas->format, 0xAF, 0xAF, 0xAF, 0xDD));
		applyTextToSurface(2 * msf, 2 * msf, canvas, "Welcome to your first level!", fontNormal, &black, 0xff);
		applyTextToSurface(2 * msf, 15 * msf, canvas, "Let's get started! To clear a level ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 24 * msf, canvas, "you have to collect all the stars on ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 33 * msf, canvas, "the grid you see in the middle. You ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 42 * msf, canvas, "can do that by telling your robot ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 51 * msf, canvas, "where to move. Start simply by ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 60 * msf, canvas, "clicking on the squares down below ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 69 * msf, canvas, "and selecting the arrow. This will ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 78 * msf, canvas, "tell the robot to make a step forward. ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 87 * msf, canvas, "Then just press the upper button on the ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 96 * msf, canvas, "left to start the program.", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 105 * msf, canvas, "CLICK TO CLOSE", fontTiny, &black, 0xff);
		popupHandler->addPopup(preparePopup(45 * msf, 2 * msf, canvas));

		//popupHandler->addPopup("Welcome to your first level!Let's get started! To clear a level you have to collect all the stars on the grid you see in the middle. You can do that by telling your robot where to move. Start simply by clicking on the squares down below and selecting the arrow. This will tell the robot to make a step forward. Then just press the upper button on the left to start the program.CLICK TO CLOSE");

		break;
	case 2:
		canvas = SDL_CreateRGBSurface(0, 98 * msf, 24 * msf, SCREEN_BPP, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
		SDL_FillRect(canvas, NULL, SDL_MapRGBA(canvas->format, 0xAF, 0xAF, 0xAF, 0xbb));
		applyTextToSurface(2 * msf, 2 * msf, canvas, "Use the new instruction to", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 11 * msf, canvas, "turn the robot to the right", fontSmall, &black, 0xff);
		popupHandler->addPopup(preparePopup(45 * msf, 2 * msf, canvas));
		break;
	case 3:
		canvas = SDL_CreateRGBSurface(0, 150 * msf, 33 * msf, SCREEN_BPP, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
		SDL_FillRect(canvas, NULL, SDL_MapRGBA(canvas->format, 0xAF, 0xAF, 0xAF, 0xbb));
		applyTextToSurface(2 * msf, 2 * msf, canvas, "With instruction that has 1 as a simbol", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 11 * msf, canvas, "you can tell robot to repeat everything ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 20 * msf, canvas, "from begining", fontSmall, &black, 0xff);
		popupHandler->addPopup(preparePopup(45 * msf, 2 * msf, canvas));
		break;
	case 4:
		canvas = SDL_CreateRGBSurface(0, 150 * msf, 73 * msf, SCREEN_BPP, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
		SDL_FillRect(canvas, NULL, SDL_MapRGBA(canvas->format, 0xAF, 0xAF, 0xAF, 0xbb));
		applyTextToSurface(2 * msf, 2 * msf, canvas, "But what instruction with number does", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 15 * msf, canvas, "is actually repeats the whole line of", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 24 * msf, canvas, "instructions. When this line ends it", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 33 * msf, canvas, "resumes previous line. ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 42 * msf, canvas, "To see how this works go to options and", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 51 * msf, canvas, "switch queue on. You can also slow down ", fontSmall, &black, 0xff);
		applyTextToSurface(2 * msf, 60 * msf, canvas, "the program.", fontSmall, &black, 0xff);
...
void fetchHelp(char *levelName, char *packName) {
	...
	if (matchLevel(levelName, packName, "Long distance", "Repetition")) genHelp(3);
	if (matchLevel(levelName, packName, "Introduction", "Functions")) genHelp(4);
